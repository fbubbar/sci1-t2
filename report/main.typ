...
#ind We are now ready to discuss the asymmetric top ($I_1 > I_2 > I_3$). Considering #eqr(<energy_ellipsoid>) and #eqr(<momentum_ellipsoid>) carefully, we notice that for a particular magnitude of $vbu(L)$, the sphere is a fixed size, and the initial direction of $vbu(L)$ determines the shape of the ellipsoid. When an asymmetric top is rotated about its tertiary axis ($I_3$), the smallest axis of the ellipsoid matches the radius of the sphere and the result is analogous to @symmetric_top_ellipsoid. The sphere lies within the ellipsoid and they intersect at two points. Rotation around the primary axis ($I_1$) is similar but now the ellipsoid lies within the sphere, as shown in @primary_axis_ellipsoid. In both of these cases, the motion is stable because small perturbations to the axis of rotation result in small intersection curves.
...
#figure(
  image("ellipsoids/intermediate_axis.png", width: 80%),
  caption: short-caption(
...
#figure(
  image("figs/output.png", width: 90%),
  caption: short-caption(
    [Identifying the precession period from the $omega_2$-$t$ graph],
    [Using interpolation to identify zeroes (blue) and a moving average to identify local minima and maxima (green), we labelled each $omega_2$-$t$ graph to calculate the precession period.]
  )
) <algorithm>
...
$<labe> using a threshold value of $theta.alt = 110%$, determined heuristically from a variety of $omega$-$t$ graphs and surprisingly small for such unstable motion.
...
#figure(
  ...
  caption: short-caption(
    ...
    [The filtered period data were fitted against the initial angular velocity \ using a three-parameter inverse model, as shown in the legend. The residuals \ appear random and the model is a good fit with $chi^2 = 1.58$.]
  )
) <model_fit>
...
#ind We compared numerical predictions from Euler's rotation equations to empirical gyroscope data for the rotation of an asymmetric top about its intermediate axis. The data indicate that the model is a good fit. Using measured moments of inertia, we verified that angular momentum is conserved except for small measurable effects due to external drag moments. The numerical model consistently predicted the period of unstable motion within 20% and further analysis revealed an inverse relationship ($chi^2 = 1.58$) between the unstable period and initial angular speed in the body frame. We also provided an intuitive explanation of the intermediate axis theorem using only basic physical principles and geometry to convey the origin of this effect to introductory physics students. 


#pagebreak()
= Acknowledgements

We appreciate the help of all those who proofread the initial drafts of the paper, including Alena Wang, Jack Honeyman, and Keanu Chan. Thanks to Chris Waltham and Jonathan Massey-Allard for valuable discussions on the project, and to Raishar Shayan for advice about data processing. We are indebted to Keanu Chan for suggesting the use of zip-ties, which saved a lot of time collecting data. Lastly, we are grateful to Brian Marcus for his mentorship and support with this project.

= Materials

All data, source code, figures, and other materials used in this paper are available from #linebreak(justify: true) https://github.com/fbubbar/sci1-t2. The repository also contains animations that visualise the rotation of an asymmetric top and its sphere-ellipsoid $L$-state space over time.
...
$ in a separate experiment. To determine $I_1$ and $I_2$, we devised a physical pendulum using a dowel as shown in @physical_pendulum_setup. We then adapted the method of #box[#cite(<goodwill_dynamics_2002>, form: "prose")] to determine the moment of inertia from the period. We measured the period using phyphox with a labelling algorithm analogous to the one shown in @algorithm. The rotational inertia about the butt of the racquet was used to find $I_1$ and $I_2$ about the centre of mass using $
...
#figure(
  image("figs/physical_pendulum.jpg", height: 42.6%),
  caption: short-caption(
...
#ind Calibration of the torsional pendulum determined a torsional constant of #box[$kappa = unit((1.347 plus.minus 0.050) times 10^(-6), N dot m)$] from the values given in @torsional_calibration. Uncertainties in the phone mass and density were assumed to be negligible compared to the uncertainty in the period. The results of the torsional and physical pendulum trials are shown in @pendulum_data.  
